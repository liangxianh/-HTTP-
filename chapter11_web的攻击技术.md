...
1. 密码破解: 该攻击即算出密码，突破认证；该攻击不仅限于web应用，还包括其他系统（如FTP或SSH等）；有以下两种手段：

* 通过网络的密码试错

a. 穷举法
b. 字典攻击：利用实现收集好的候选密码（经过各种组合方式后存入字典）

字典攻击中有一种是利用其他web网站已泄漏的id及密码列表进行攻击，根据用户习惯，这种方式攻击成功的概率非常高；

* 对已加密的密码的破解（指攻击者入侵系统，已获得加密或散列处理的密码数据的情况）

web应用在保存密码时，一般不会以明文形势，通过散列函数（例如MD5）做散列处理或加salt的手段对要保存的密码本身加密；即使攻击者获取到该内容，也需要进行解码还原成明文才可以使用；通常有以下几种方法
[MD5维基百科](https://en.wikipedia.org/wiki/MD5)

a. 通过穷举法/字典攻击进行类推
```
攻击者拿到的散列值： 9001jdkj9794830kdjfkdjf772

调用散列函数对候选密码进行散列处理后类推 

a   ----          ----> asdfasdf
b   ----          ----> asdfasdf
c   ----          ----> asdfasdf
……  ---- 散列函数  ----> dfasfdasdf
abc ----  MD5     ----> 9001jdkj9794830kdjfkdjf772 经对比一致
……  ----          ----> dfasdfa
x   ----          ----> sdfasdfasdf
y   ----          ----> dsfasdfasdf
z   ----          ----> sdfadfasdf
...
b. 彩虹表：使用明文密码及与之对应的散列值构成的一张数据库表；

|  明文   | 散列值(MD5)  |
|  ----  | ----  |
| a      | dfasdfasdf4534g345243xf245 |
| abc    | 9001jdkj9794830kdjfkdjf772 |

[这里有很多的彩虹表奥](https://freerainbowtables.com/)

c. 拿到密码

使用共享密钥加密方式对密码数据进行加密处理的情况下，如果能获取加密使用的密钥，也就可以对密码数据进行解密了；

d. 加密算法的漏洞


2. 点击劫持：是指利用透明的按钮或者链接做成陷阱，覆盖在web页面之上；然后诱使用户在不知情的情况下，点击链接访问内容的一种攻击手段；又称为界面伪造（UI Redressing）

> 点击劫持的攻击案例

以SNS网站的注销为例，该功能注册登陆的SNS用户只需要点击注销按钮，就可以从SNS网站上注销自己的会员身份；
```
a：攻击者在预料用户会点击的web页面上设下陷阱；
b: 目标的sns网站将作为透明层覆盖在游戏页面上；覆盖时需要注意点击按钮的重合；
iframe页面中使用的透明可点击按钮的示例；
...
3. DoS攻击：是一种让运行中的服务呈停止状态的攻击，（也称服务停止攻击或拒绝服务攻击）；
该攻击不仅对web网站，还包括网络设备及服务器等；主要有以下两种攻击方式；

* 集中利用访问请求造成资源过载，资源用尽的同时，实际上服务也呈停止状态；
单纯来讲就是发送大量的合法请求；服务器很难分辨何为正常请求，何为攻击请求，因此很难防止dos攻击；

多台计算机发起的Dos攻击称为DDos攻击（Distributed Denial of Servive attack）
DDos攻击通常利用那些感染病毒的计算机作为攻击者的攻击跳板；

* 通过攻击安全漏洞使服务停止；


4. 后门程序： 开发设置的隐藏入口，可不按正常步骤使用受限功能；利用后门程序就可以使用原本受限制的功能
分为以下三种类型：
...
可通过监视进程和通信状态发现被植入的后门程序；但是设置在web应用中的后门程序，由于和正常使用时区别不大，而很难发现；
